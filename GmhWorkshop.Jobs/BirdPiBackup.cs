...
namespace GmhWorkshop.Jobs
{
    public static class BirdPiBackup
    {
        public static async Task Run(WorkshopSettings settings, IProgress<string> progress)
        {
            ...
            await Sftp.DownloadDirectoriesAndRegularFiles(sftpClient, Path.Combine(settings.BirdPiRemoteHomeDirectory, "BirdSongs/Extracted/By_Date"),
                Path.Combine(backupDirectory.FullName, "Extracted", "By_Date"), progress);
            await Sftp.DownloadDirectoriesAndRegularFiles(sftpClient, Path.Combine(settings.BirdPiRemoteHomeDirectory, "BirdSongs/Extracted/Charts"),
